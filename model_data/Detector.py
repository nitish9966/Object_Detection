...
np.random.seed(20)
class Detector:
    def __init__(self,videPath,configPath,modelPath,classesPath):
        ...
        while success:
            ...
            if len(bboxsIdx)!=0:
                for i in range(0,len(bboxsIdx)):
                    ...
                    dispalyText="{}:{:.2f}".format(classLabel,classConfidence)


                    x,y,w,h=bbox

                    cv2.rectangle(image,(x,y),(x+w, y+h),color=classColor,thickness=1)
                    cv2.putText(image,dispalyText,(x,y-10),cv2.FONT_HERSHEY_PLAIN,1,classColor,2)
